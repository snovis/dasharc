        ...
        return (
          <div key={p.dataKey} className="flex items-center justify-between gap-4 py-0.5">
            ...
            <span className="font-medium text-slate-800">
              {p.value} <span className="text-slate-400">({pct}%)</span>
            </span>
          </div>
        )
      })}
...
export default function CallOutcomesChart({ data, onAgentClick }) {
  ...
  return (
    <ResponsiveContainer width="100%" height={Math.max(180, data.length * 72)}>
      <BarChart
        data={data}
        layout="vertical"
      ...
      >
        ...
        <YAxis
          ...
          dataKey="name"
          width={160}
          tick={{ fontSize: 12, fill: '#475569', cursor: onAgentClick ? 'pointer' : 'default' }}
          tickLine={false}
          axisLine={false}
        ...
        {OUTCOME_BUCKETS.map((o, i) => (
          <Bar
            ...
            stackId="a"
            fill={o.color}
            radius={i === OUTCOME_BUCKETS.length - 1 ? [0, 3, 3, 0] : 0}
