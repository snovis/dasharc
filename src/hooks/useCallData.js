...
export function useDallasStats({ fromDate, toDate }) {
  const { isAuthenticated } = useAuth()

  return useQuery({
    queryKey: ['dallas-stats', fromDate, toDate],
    queryFn: async () => {
      const params = new URLSearchParams()
      if (fromDate) params.set('fromDate', fromDate)
      if (toDate) params.set('toDate', toDate)
      return authedFetch(`/api/dallas-stats?${params}`)
    },
    enabled: isAuthenticated && !!fromDate && !!toDate,
    staleTime: 1000 * 60 * 5,
  })
}
